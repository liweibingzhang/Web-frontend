...
@Component({
  selector: 'app-structure-component-dfs',
  imports: [
    CommonModule, 
    FormsModule, 
    ...materialModules, 
  ...
  templateUrl: './structure-component-dfs.component.html',
  styleUrl: './structure-component-dfs.component.css'
})
export class DfsComponent implements AfterViewInit, OnInit {
  ...
  ngOnInit() {
    ...
    // 绑定方法到当前组件实例
    this.generateDFSSteps = this.generateDFSSteps.bind(this);
  ...
  generateDFSSteps = () => {
    ...
    const stack: string[] = ['0'];  // 从根节点开始，使用栈而不是队列
    ...
    while (stack.length > 0) {
      const currentNode = stack.pop()!; // 使用pop()而不是shift()
      if (!visited.has(currentNode)) {
        ...
        // 获取当前节点的所有未访问邻居（反向遍历以保持从左到右的访问顺序）
  ...
  getDFSDescription(): { algorithm: string, visualization: string } {
    return {
      algorithm: `深度优先搜索(DFS)的时间复杂度为 O(V + E)
具体步骤：
1. 从根节点开始，将其加入栈
2. 取出栈顶节点，访问该节点 
3. 将该节点的所有未访问的邻接节点加入栈
4. 重复步骤2-3，直到栈为空`,
      visualization: `
可视化元素说明：
- 蓝色节点：未访问节点
